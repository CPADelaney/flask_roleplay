# lore/managers/conflict_manager.py
...
class ConflictManager(BaseLoreManager):
    """
    Manager for national and international conflicts, domestic issues,
    and news generation within a matriarchal society.
    """
    
    def __init__(self, user_id: int, conversation_id: int):
        super().__init__(user_id, conversation_id)
        self.cache_namespace = "conflict"
    
    async def _initialize_tables(self):
        """Initialize tables needed for conflicts and issues"""
        table_definitions = {
            "NationalConflicts": """
    ...
    @with_governance(
        ...
        id_from_context=lambda ctx: "conflict_manager"
    )
    async def generate_domestic_issues(self, ctx, nation_id: int, count: int = 2) -> List[Dict[str, Any]]:
        ...
        for issue_type in selected_types:
            ...
            try:
                ...
                issue_data["description"] = MatriarchalThemingUtils.apply_matriarchal_theme(
                    "social_structure", issue_data["description"], emphasis_level=1
                )
    ...
    async def _generate_domestic_news(
        ...
        for bias in biases:
            ...
            try:
                ...
                # Apply matriarchal theming
                news_data["content"] = MatriarchalThemingUtils.apply_matriarchal_theme(
                    "news", news_data["content"], emphasis_level=1
                )
    ...
    async def generate_initial_conflicts(self, ctx, count: int = 3) -> List[Dict[str, Any]]:
        ...
        geo_manager = GeopoliticalSystemManager(self.user_id, self.conversation_id)
        await geo_manager.ensure_initialized()
        nations = await geo_manager.get_all_nations(run_ctx)
        ...
        for i in range(count):
            ...
            try:
                ...
                # Apply matriarchal theming
                conflict_data["description"] = MatriarchalThemingUtils.apply_matriarchal_theme(
                    "conflict", conflict_data["description"], emphasis_level=1
                )
    ...
    async def evolve_all_conflicts(self, ctx, days_passed: int = 30) -> Dict[str, Any]:
        ...
        geo_manager = GeopoliticalSystemManager(self.user_id, self.conversation_id)
        await geo_manager.ensure_initialized()
        all_nations = await geo_manager.get_all_nations(run_ctx)
    ...
    async def register_with_governance(self):
        ...
        await super().register_with_governance(
            ...
            directive_text="Manage, generate, and evolve national conflicts and domestic issues.",
