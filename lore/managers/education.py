...
class EducationalSystemManager(BaseLoreManager):
    ...
    async def stream_educational_development(
        ...
        with trace(
            ...
            leadership_result = await Runner.run(specialist_agent, leadership_prompt, context=run_ctx.context)
            leadership_structure = leadership_result.final_output
            
            yield f"\nLeadership Structure:\n{leadership_structure}\n"
            await asyncio.sleep(0.5)
            
            # 3. Stream curriculum and teaching methods
