...
class ConflictSystemIntegration:
    ...
    async def register_enhanced_integration(cls, user_id: int, conversation_id: int):
        ...
        try:
            ...
            # signature is register_agent(agent_type, agent_instance, agent_id)
            ...
            logger.info(
                f"Successfully registered conflict system integration for "
                f"user={user_id}, conversation={conversation_id}"
            )
            return {"success": True, "integration": integration}
    
        except Exception as e:
            logger.error(f"Error registering conflict system integration: {e}", exc_info=True)
            return {"success": False, "message": str(e)}
