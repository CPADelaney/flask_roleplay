...
# logic/conflict_system/hooks.py
"""
Simple hooks for easy integration with the conflict system
"""
...
class ConflictHooks:
    """Simple hooks for conflict system integration"""
    
    @staticmethod
    async def check_and_generate(user_id: int, conversation_id: int) -> Optional[Dict[str, Any]]:
        """Check if conflict should be generated and create if appropriate"""
        system = await ConflictSystemIntegration.get_instance(user_id, conversation_id)
        return await system.check_and_generate_conflicts()
    
    @staticmethod
    async def get_player_conflicts(user_id: int, conversation_id: int) -> List[Dict[str, Any]]:
        """Get conflicts involving the player"""
        system = await ConflictSystemIntegration.get_instance(user_id, conversation_id)
        conflicts = await system.get_active_conflicts()
        
        return [c for c in conflicts 
                if c.get('player_involvement') 
                and c['player_involvement'].get('involvement_level') != 'none']
    
    @staticmethod
    async def advance_story(user_id: int, conversation_id: int,
                          conflict_id: int, event: str,
                          npcs: Optional[List[int]] = None) -> Dict[str, Any]:
        """Advance conflict story"""
        system = await ConflictSystemIntegration.get_instance(user_id, conversation_id)
        return await system.advance_conflict(conflict_id, event, npcs)
    
    @staticmethod
    async def trigger_from_event(user_id: int, conversation_id: int,
                               event_type: str, event_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Trigger conflict from game event"""
        system = await ConflictSystemIntegration.get_instance(user_id, conversation_id)
        
        # Add event pressure
        if event_type == "relationship_breakdown":
            return await system.generate_conflict("personal_dispute")
        elif event_type == "faction_power_shift":
            return await system.generate_conflict("faction_rivalry")
        elif event_type == "resource_crisis":
            return await system.generate_conflict("economic_collapse")
            
        return None
    
    @staticmethod
    async def get_summary(user_id: int, conversation_id: int,
                        conflict_id: int) -> Dict[str, Any]:
        """Get conflict summary for display"""
        system = await ConflictSystemIntegration.get_instance(user_id, conversation_id)
        conflict = await system.get_conflict(conflict_id)
        ...
        return {
            ...
            "key_players": [s['npc_name'] for s in conflict['stakeholders'][:3]]
        }
    
    @staticmethod
    async def get_tension_level(user_id: int, conversation_id: int) -> str:
        """Get world tension level"""
        system = await ConflictSystemIntegration.get_instance(user_id, conversation_id)
        analysis = await system.generator.analyzer.analyze_world_state()
...
# Event handlers for other systems to use

async def on_relationship_change(user_id: int, conversation_id: int,
                               npc1_id: int, npc2_id: int,
                               old_level: int, new_level: int):
    """Called when NPC relationships change"""
    if old_level > 0 and new_level < -50:
        await ConflictHooks.trigger_from_event(
            user_id, conversation_id,
            "relationship_breakdown",
            {"npc1": npc1_id, "npc2": npc2_id}
        )

async def on_faction_shift(user_id: int, conversation_id: int,
                         faction: str, power_change: int):
    """Called when faction power shifts"""
    if abs(power_change) > 5:
        await ConflictHooks.trigger_from_event(
            user_id, conversation_id,
            "faction_power_shift",
            {"faction": faction, "change": power_change}
        )

async def on_resource_scarcity(user_id: int, conversation_id: int,
                             resource: str, severity: str):
    """Called when resources become scarce"""
    if severity in ["critical", "severe"]:
        await ConflictHooks.trigger_from_event(
            user_id, conversation_id,
            "resource_crisis",
