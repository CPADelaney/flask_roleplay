...
"""
Handles NPC perception of their environment, plus action execution results.
"""
...
async def fetch_environment_data(
    ...
    env_data = {
        ...
        "entities_present": []  # type: List[Dict[str, Any]]
    }

    # If location wasn't explicitly in context, try to fetch from DB
    if env_data["location"] == "Unknown":
        with get_db_connection() as conn, conn.cursor() as cursor:
            ...
            # If we now have a location, fetch entities in that location
            if env_data["location"] != "Unknown":
                entity_query = """
...
async def execute_npc_action(
    ...
    Args:
        ...
        action: Dictionary with keys like "type", "description", "target"
    ...
    Returns:
        A dictionary with:
          ...
          - "target_reactions": list of strings describing target responses
    """
    result = {
    ...
    with get_db_connection() as conn, conn.cursor() as cursor:
        ...
        # Now handle action types
        action_type = action.get("type")
        target = action.get("target", "group")

        if action_type == "talk":
            result.update(_handle_talk_action(npc_name, target, cursor, user_id, conversation_id))

        elif action_type == "command":
            result.update(_handle_command_action(npc_name, target, cursor, user_id, conversation_id))

        elif action_type == "mock":
            result.update(_handle_mock_action(npc_name, target, cursor, user_id, conversation_id))
            ...
            result["outcome"] = f"{npc_name} {action.get('description','does something scheduled')}."
            result["emotional_impact"] = 0
...
def _handle_talk_action(
    ...
    """
    Handle the 'talk' action specifics.
    """
    result = {
        "outcome": "",
        "emotional_impact": 1,
    ...
    if target == "player":
        ...
        # assume target is an NPC ID
        try:
            target_id = int(target)
            target_name = _fetch_target_name(cursor, target_id, user_id, conversation_id)
            result["outcome"] = f"{npc_name} talks to {target_name}."
            result["target_reactions"] = [f"{target_name} listens"]
        except ValueError:
            # if the target isn't a valid ID
            result["outcome"] = f"{npc_name} tries to talk, but the target is unclear."
...
def _handle_command_action(
    ...
    """
    Handle the 'command' action specifics.
    """
    result = {
        "outcome": "",
        "emotional_impact": 2,
    ...
    if target == "player":
        ...
        try:
            ...
            result["outcome"] = f"{npc_name} issues a command, but the target is unclear."
...
def _handle_mock_action(
    ...
    """
    Handle the 'mock' action specifics.
    """
    result = {
        "outcome": "",
        "emotional_impact": -2,
    ...
    if target == "player":
        ...
        try:
            ...
            result["outcome"] = f"{npc_name} attempts to mock someone, but the target is unclear."
