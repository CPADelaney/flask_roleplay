...
def start_new_game():
    logging.info("=== /start_new_game endpoint called (waiting for background processing) ===")
    ...
    # Wait for the task to finish (with a timeout, e.g. 60 seconds)
    try:
        result = task.get(timeout=60)
        logging.info("Background task completed: %s", result)
        return jsonify(result), 200
    except Exception as e:
        logging.exception("Error waiting for background task:")
