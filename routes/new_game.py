...
def start_new_game():
    """
    Offload heavy GPT processing and database updates to a background task.
    This endpoint immediately returns a job_id that the client can later poll.
    """
    ...
    # Get any request data needed for starting the game.
    conversation_data = request.get_json() or {}
    
    # Enqueue the background task.
    task = process_new_game_task.delay(user_id, conversation_data)
    logging.info(f"Enqueued process_new_game_task for user_id={user_id} with task id: {task.id}")
    
    # Immediately return a job_id for polling task status.
