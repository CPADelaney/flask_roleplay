"""Core configuration helpers for Nyx."""
...
DEFAULT_CONFIG: Dict[str, Dict[str, Any]] = {
    ...
    "decision_engine": {
        ...
        "model_name": "gpt-5-nano",  # Model to use for decision engine
...
def _build_config() -> Dict[str, Any]:
    """Create a configuration dictionary including environment overrides."""
    ...
    for section_key, section_values in config.items():
        if not isinstance(section_values, dict):
            # Configuration sections should be dicts, but we guard anyway.
            continue
        ...
        for key, value in section_values.items():
            ...
            if isinstance(value, bool):
                section_values[key] = env_value.lower() in ("true", "yes", "1")
            elif isinstance(value, int):
                section_values[key] = int(env_value)
            elif isinstance(value, float):
                section_values[key] = float(env_value)
            else:
                section_values[key] = env_value
