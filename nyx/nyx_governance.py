...
class NyxUnifiedGovernor:
    ...
    async def _generate_interaction_guidance(
        ...
        return guidance
# ENHANCEMENT IMPLEMENTATIONS FOR EXISTING NYX GOVERNANCE SYSTEM
# Add these methods to your existing classes

# -----------------------------------------------------------------------------
# ADDITIONS TO nyx/nyx_governance.py - NyxUnifiedGovernor class
# -----------------------------------------------------------------------------
...
async def _are_actions_compatible(self, action1: Dict[str, Any], action2: Dict[str, Any]) -> bool:
...
async def generate_agent_feedback(
    ...
    rejected_actions = [a for a in recent_actions if a.get("approved", True) == False]
    if len(rejected_actions) > 2:
        # Pattern of rejected actions - suggest improvements
        rejection_reasons = [a.get("reasoning", "") for a in rejected_actions]
    ...
    try:
        async with asyncpg.create_pool(dsn=get_db_connection()) as pool:
            async with pool.acquire() as conn:
                if agent_type == AgentType.NPC:
                    rows = await conn.fetch("""
                        SELECT action_data, result_data, status, timestamp
                        FROM NyxActionTracking
                        WHERE user_id = $1 AND conversation_id = $2 AND npc_id = $3
                        ORDER BY timestamp DESC
                        LIMIT 10
...
# 3. Dynamic Agent Discovery & Registration
...
# Add an instance of the SharedGameState to the NyxUnifiedGovernor
async def initialize_game_state(self):
    """Initialize the shared game state."""
    self.game_state = SharedGameState(self.user_id, self.conversation_id)
...
async def ensure_temporal_consistency(
    ...
    for dependency in proposed_action.get("dependencies", []):
...
async def advance_time(self, time_increment: str = "1 hour") -> Dict[str, Any]:
    ...
    update_directives = []
    for agent_type, agents in self.registered_agents.items():
...
# 7. Memory-Based Decision Making
...
def _calculate_success_rate(self, actions: List[Dict[str, Any]]) -> float:
    ...
    successful = sum(
        ...
        if action.get("result_data", {}).get("success", False) or 
...
def _calculate_response_time(
    ...
    """Calculate average response time to directives."""
    if not actions or not directives:
        return 0.0
    
    # Match actions to directives by looking at the description
    # This is a simplified approach - a real implementation would use proper tracking
    response_times = []
    
    for directive in directives:
        directive_text = json.dumps(directive).lower()
        directive_time = datetime.fromisoformat(directive.get("timestamp", datetime.now().isoformat()))
        
        for action in actions:
            action_text = json.dumps(action).lower()
            action_time = action.get("timestamp", datetime.now())
            
            # Simple matching - check if directive text appears in action
            if any(word in action_text for word in directive_text.split()[:5]):
                ...
                response_time = (action_time - directive_time).total_seconds() / 60
                if 0 <= response_time <= 60:  # Reasonable range
...
async def _calculate_functional_consistency(self, agent_type: str, agent_id: Union[int, str]) -> float:
    ...
    action_types = [action.get("action_data", {}).get("type", "unknown") for action in actions]
...
# Add these setup methods to NyxUnifiedGovernor.__init__
...
async def setup_database_tables(self):
    ...
    async with asyncpg.create_pool(dsn=get_db_connection()) as pool:
        async with pool.acquire() as conn:
            ...
            logger.info("Created enhanced NyxUnifiedGovernor database tables")
