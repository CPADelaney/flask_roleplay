...
class NeedsSystem:
            ...
            if elapsed_hours > 0.001:  # Only update if ~4 seconds passed
                for name, need in self.needs.items():
                    ...
                    if (len(self.need_history[name]) == 0 or 
                        ...
                        self._add_history_entry(name, need, "decay")
                    ...
                    if drive > self.drive_threshold_for_goal:
                        ...
                        if not self.goal_manager or not self.goal_manager.has_active_goal_for_need(name):
                ...
                if needs_to_trigger_goals and self.goal_manager:
                     await self._trigger_goal_creation(needs_to_trigger_goals)
            else:
    ...
    def _add_history_entry(self, need_name: str, need: NeedState, reason: str) -> None:
        ...
        self.need_history[need_name].append(entry)
        
        # Trim history if needed
        if len(self.need_history[need_name]) > self.max_history_per_need:
            self.need_history[need_name] = self.need_history[need_name][-self.max_history_per_need:]

    async def _trigger_goal_creation(self, needs_list: List[NeedState]) -> List[str]:
    ...
    async def satisfy_need(self, name: str, amount: float, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        Args:
            ...
            context: Optional context affecting satisfaction.
        """
        ...
        async with self._lock:
            ...
            # Add to history
            self._add_history_entry(name, need, reason)
    ...
    def get_needs_by_category(self) -> Dict[str, Dict[str, Dict[str, Any]]]:
        ...
        for category, need_names in self.need_categories.items():
            result[category] = {
    ...
    async def get_most_unfulfilled_need(self) -> Dict[str, Any]:
        ...
        await self.update_needs()  # Ensure needs are current
    ...
    async def reset_need_to_default(self, need_name: str) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            original = self.needs[need_name].model_dump()
            ...
            # Add to history
            self._add_history_entry(need_name, self.needs[need_name], "reset_to_default")
            
            return {
                "status": "success",
                "need": need_name,
                "original": original,
                "new": self.needs[need_name].model_dump()
            }
