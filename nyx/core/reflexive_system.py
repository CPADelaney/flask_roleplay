...
class ReflexiveSystem:
    ...
    async def get_reflexive_stats(self) -> Dict[str, Any]:
        ...
        for domain, patterns in self.domain_libraries.items():
            if patterns:
                reaction_times = [
                    rt for p in patterns.values() for rt in p.response_times
                ]
...
# Integration with main NyxBrain class
async def initialize_reflexive_system(self):
    """Initialize reflexive system integration with NyxBrain"""
    self.reflexive_system = ReflexiveSystem(self.agent_enhanced_memory)
    
    # Register reflexive system tools
    # These will be added to the brain_agent
    self.register_reflex = self.reflexive_system.register_reflex
    self.process_stimulus_fast = self.reflexive_system.process_stimulus_fast
    self.train_reflexes = self.reflexive_system.train_reflexes
    self.add_gaming_reflex = self.reflexive_system.add_gaming_reflex
    self.simulate_gaming_scenarios = self.reflexive_system.simulate_gaming_scenarios
    self.set_reflex_response_mode = self.reflexive_system.set_response_mode
    self.get_reflexive_stats = self.reflexive_system.get_reflexive_stats
    self.optimize_reflexes = self.reflexive_system.optimize_reflexes
    
    logger.info("Reflexive system initialized and integrated with brain")
    
    return self.reflexive_system
