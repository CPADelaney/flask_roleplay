    ...
    async def run_maintenance(self) -> Dict[str, Any]:
            ...
            with trace(workflow_name=workflow_name, group_id=group_id):
                ...
                try:
                    ...
                    logger.info(f"Maintenance Runner completed. Last agent: {result.last_agent.name if result.last_agent else 'Unknown'}. Final output type: {output_type_name}")
                    ...
                    if isinstance(final_output_obj, MaintenanceSummaryOutput):
                        ...
                        # Optionally, log the actual output if simple
                        if isinstance(final_output_obj, (str, dict, list, int, float, bool)):
                            logger.warning(f"Actual final output content: {str(final_output_obj)[:500]}") # Log truncated output
    
                    # Update last maintenance time *after* the run completes
                    ...
                    failure_record = {
                        ...
                        "duration_seconds": duration,
                        "error": str(e),
                        "traceback": traceback.format_exc(), # Add traceback if needed
                        "status": "failed",
                    ...
                    return failure_record # Return the failure details
                finally:
