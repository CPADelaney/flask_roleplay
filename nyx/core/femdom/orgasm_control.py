...
class OrgasmControlSystem:
    """Manages denial, edging, and permission dynamics."""
    ...
    def __init__(self, reward_system=None, memory_core=None, relationship_manager=None, somatosensory_system=None):
        ...
        self.permission_states: Dict[str, UserPermissionState] = {}
        
        # Active denial periods
        self.denial_periods: Dict[str, List[DenialPeriod]] = {}
        
        # Begging records
        self.begging_records: Dict[str, List[BeggingRecord]] = {}
        
        # Orgasm records
        self.orgasm_records: Dict[str, List[OrgasmRecord]] = {}
    ...
    async def initialize_user(self, user_id: str) -> UserPermissionState:
        ...
        async with self._lock:
            if user_id in self.permission_states:
                return self.permission_states[user_id]
            
            # Create new user state
            user_state = UserPermissionState(user_id=user_id)
            
            # Initialize empty record lists
            self.begging_records[user_id] = []
            self.orgasm_records[user_id] = []
            self.denial_periods[user_id] = []
            
            # Store state
            self.permission_states[user_id] = user_state
    ...
    async def process_permission_request(self, 
        ...
        if user_id not in self.permission_states:
            await self.initialize_user(user_id)
        
        user_state = self.permission_states[user_id]
        
        # Check current status
        current_status = user_state.current_status
        
        # Analyze begging/desperation level
        desperation_level = self._analyze_desperation(request_text)
        ...
        async with self._lock:
            ...
            self.begging_records[user_id].append(begging_record)
            ...
            if user_state.denial_active and user_state.current_denial_period:
                ...
                for period in self.denial_periods[user_id]:
            ...
            if in_denial and active_denial:
                ...
                if current_status == PermissionStatus.DENIED:
                    ...
                    permission_granted = False
                    reason = "You may edge only, no orgasm permitted"
            ...
            response = self._generate_permission_response(
                user_id, 
                permission_granted, 
                reason, 
                desperation_level, 
            ...
            somatic_result = None
            if permission_granted and self.somatosensory_system:
                try:
                    # Process positive sensation from granting permission
                    somatic_result = await self.somatosensory_system.process_stimulus(
    ...
    def _analyze_desperation(self, request_text: str) -> float:
        ...
        high_count = sum(text_lower.count(word) for word in self.desperation_keywords["high"])
        medium_count = sum(text_lower.count(word) for word in self.desperation_keywords["medium"])
        low_count = sum(text_lower.count(word) for word in self.desperation_keywords["low"])
    ...
    async def start_denial_period(self, 
        ...
        async with self._lock:
            ...
            self.denial_periods[user_id].append(denial_period)
            ...
            message = self._generate_denial_start_message(level, duration_hours, begging_allowed)
    ...
    async def extend_denial_period(self, 
        ...
        async with self._lock:
            ...
            message = self._generate_extension_message(level_name, additional_hours, reason)
    ...
    def _generate_extension_message(self, level_name: str, additional_hours: int, reason: str) -> str:
    ...
    async def end_denial_period(self, 
        ...
        async with self._lock:
            ...
            # Generate message
            message = self._generate_denial_end_message(target_denial.level.name, duration_hours, reason)
    ...
    def _generate_denial_end_message(self, level_name: str, duration_hours: float, reason: str) -> str:
    ...
    async def record_orgasm(self, 
        ...
        async with self._lock:
            ...
            self.orgasm_records[user_id].append(record)
            ...
            # Generate message
            message = self._generate_orgasm_message(orgasm_type, with_permission, denial_ended)
    ...
    def _generate_orgasm_message(self, orgasm_type: str, with_permission: bool, denial_ended: bool) -> str:
    ...
    async def set_permission_status(self, 
        ...
        async with self._lock:
            ...
            # Generate message
            message = self._generate_status_change_message(old_status, status, reason)
    ...
    async def apply_permission_template(self, 
        ...
        if status == PermissionStatus.DENIED:
            # Start denial period
            result = await self.start_denial_period(
                user_id=user_id,
                duration_hours=duration_hours,
                level=level,
                begging_allowed=begging_allowed
            ...
            result = await self.set_permission_status(
                user_id=user_id,
                status=status,
    ...
    async def get_permission_state(self, user_id: str) -> Dict[str, Any]:
        ...
        recent_begging = []
        for record in self.begging_records.get(user_id, [])[-5:]:  # Last 5
        ...
        recent_orgasms = []
        for record in self.orgasm_records.get(user_id, [])[-5:]:  # Last 5
    ...
    async def create_permission_template(self, template_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Store template
            self.permission_templates[template_name] = template
    ...
    async def analyze_control_patterns(self, user_id: str) -> Dict[str, Any]:
        ...
        # Get user data
        begging_records = self.begging_records.get(user_id, [])
        orgasm_records = self.orgasm_records.get(user_id, [])
        denial_periods = self.denial_periods.get(user_id, [])
        user_state = self.permission_states[user_id]
