...
# -------------------------------------------------------------------------------
# Enhanced Context Cache with Intelligent Eviction
# -------------------------------------------------------------------------------

class EnhancedContextCache:
    """Multi-level cache with intelligent eviction and compression strategies"""
    ...
    def __init__(self):
        ...
        # Level 3: Long-lived (hours to days)
        self.l3_cache = {}
        ...
        self.l3_timestamps = {}
        self.l3_access_count = {}
        
        # Importance scores - used for eviction decisions
    ...
    async def get(self, key: str, fetch_func, cache_level: int = 1, importance: float = 1.0):
        """
        Get data from cache or fetch it with intelligent caching.
        
        Args:
            key: Cache key
            fetch_func: Async function to call if cache miss
            cache_level: Desired cache level (1-3)
            importance: Importance score (0-10) for eviction decisions
        ...
        if key in self.l2_cache:
            if now - self.l2_timestamps[key] < self.l2_ttl:
                ...
                self.l1_access_count[key] = self.l2_access_count.get(key, 0) + 1
                
                # Ensure L1 cache doesn't exceed size limit
                if len(self.l1_cache) > self.l1_max_size:
                    self._evict_from_cache(self.l1_cache, self.l1_timestamps, 
                                          self.l1_access_count, 0.2)
        ...
        if key in self.l3_cache:
            if now - self.l3_timestamps[key] < self.l3_ttl:
                ...
                data = self._decompress_if_needed(self.l3_cache[key])
                
                # Promote to L2 cache
                self.l2_cache[key] = data
                self.l2_timestamps[key] = now
                self.l2_access_count[key] = self.l3_access_count.get(key, 0) + 1
                
                # Ensure L2 cache doesn't exceed size limit
                if len(self.l2_cache) > self.l2_max_size:
                    self._evict_from_cache(self.l2_cache, self.l2_timestamps, 
                                          self.l2_access_count, 0.2)
                
                return data
            else:
                del self.l3_cache[key]
                del self.l3_timestamps[key]
                if key in self.l3_access_count:
                    del self.l3_access_count[key]
        ...
        if cache_level >= 3:
            # Compress for L3 storage if it's large
            compressed_data = self._compress_if_needed(data)
            self.l3_cache[key] = compressed_data
            self.l3_timestamps[key] = now
            self.l3_access_count[key] = 1
            
            # Ensure L3 cache doesn't exceed size limit
            if len(self.l3_cache) > self.l3_max_size:
                self._evict_from_cache(self.l3_cache, self.l3_timestamps, 
                                      self.l3_access_count, 0.1)
        
        return data
    
    def _evict_from_cache(self, cache_dict, timestamps_dict, access_count_dict, ratio):
        """
        Intelligently evict items from a cache based on importance, age, and access frequency.
        
        Args:
            cache_dict: The cache dictionary to evict from
            timestamps_dict: Dictionary of timestamps for cache entries
            access_count_dict: Dictionary of access counts for cache entries
            ratio: Ratio of items to evict (0.0-1.0)
        """
        if not cache_dict:
            return
        
        # Number of items to evict
        num_to_evict = max(1, int(len(cache_dict) * ratio))
        
        # Build a list of (score, key) pairs for all items
        ...
        for key in cache_dict:
            ...
            age = now - timestamps_dict.get(key, 0)
            age_factor = math.exp(-0.01 * (age / 60))  # Normalize by minutes
            
            # Access count factor (less accessed = lower score)
            access_factor = math.log1p(access_count_dict.get(key, 0))
            
            # Importance factor (less important = lower score)
            importance = self.importance_scores.get(key, 1.0)
            
            # Combined score
            score = (0.4 * age_factor) + (0.3 * access_factor) + (0.3 * importance)
            scores.append((score, key))
        
        # Sort by score (ascending) and take the num_to_evict lowest scoring items
        scores.sort()
        keys_to_evict = [key for _, key in scores[:num_to_evict]]
        ...
        for key in keys_to_evict:
            ...
            if key in timestamps_dict:
                del timestamps_dict[key]
            if key in access_count_dict:
                del access_count_dict[key]
    
    def _compress_if_needed(self, data):
        """Compress data if it's large enough to benefit from compression"""
        # For now, just return the data as is
        # In a real implementation, you would compress large data here
        return data
    
    def _decompress_if_needed(self, data):
        """Decompress data if it was compressed"""
        # For now, just return the data as is
        # In a real implementation, you would decompress data here
        return data
    
    def invalidate(self, key_prefix=None):
        """
        Invalidate specific cache key or all keys with the given prefix.
        
        Args:
            key_prefix: Optional prefix to match for invalidation
        """
        if key_prefix is None:
            ...
            self.l3_access_count.clear()
            
            return
        
        # Invalidate by prefix
        for cache_dict, timestamp_dict, access_dict in [
            (self.l1_cache, self.l1_timestamps, self.l1_access_count),
            (self.l2_cache, self.l2_timestamps, self.l2_access_count),
            (self.l3_cache, self.l3_timestamps, self.l3_access_count)
        ]:
            keys_to_remove = [key for key in cache_dict if key.startswith(key_prefix)]
